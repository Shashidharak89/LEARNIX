  ...
  const downloadTopicAsPDF = async (data) => {
    ...
    try {
      ...
      if (data.topic.content) {
        const splitText = pdf.splitTextToSize(`Content: ${data.topic.content}`, pageWidth - 2 * margin);
      ...
      const fileName = `${data.topic.topic}_${data.subject.subject}_${data.user.name}`.replace(/[^a-zA-Z0-9]/g, '_') + '.pdf';
      pdf.save(fileName);
    } catch (error) {
